...
$p = Join-Path $Root "react-frontend\\src\\components\\tiles\\VolSurfaceTile.tsx"
$c = @'
...
Write-Host "[Granite] VolSurfaceTile.tsx patched" -ForegroundColor Cyan

Write-Host "[Granite] Building React..." -ForegroundColor Cyan
$wslRoot = ($Root -replace 'C:\\', '/mnt/c/') -replace '\\', '/'
wsl.exe bash -lc "cd '$wslRoot/react-frontend' && npm run build 2>&1 | tail -20"
if ($LASTEXITCODE -eq 0) {
    Write-Host "[Granite] Build successful!" -ForegroundColor Green
    Write-Host "Next: Ctrl+C the server, then run install_and_run_wsl.sh" -ForegroundColor Yellow
} else {
    Write-Host "[Granite] Build failed - paste errors to Claude" -ForegroundColor Red
}
