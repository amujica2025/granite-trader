...
class DXStreamer:
    ...
    async def _handle_message(self, msg: Dict[str, Any]) -> None:
        ...
        if mtype == "SETUP":
            # SETUP received - wait for AUTH_STATE UNAUTHORIZED before sending AUTH
            log.debug("SETUP handshake received - waiting for AUTH_STATE UNAUTHORIZED")

        elif mtype == "AUTH_STATE":
            state = msg.get("state", "")
            if state == "UNAUTHORIZED":
                # Per DXLink docs: AUTH must be sent in response to AUTH_STATE UNAUTHORIZED
                log.info("DXLink AUTH_STATE UNAUTHORIZED - sending AUTH token")
                await self._send({"type": "AUTH", "channel": 0, "token": self._token})
            elif state == "AUTHORIZED":
                log.info("DXLink AUTHORIZED - opening channel")
