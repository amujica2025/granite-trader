...
'@
[System.IO.File]::WriteAllText((Join-Path $Root "react-frontend\src\components\tiles\ChartTile.tsx"), $c, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "[OK] react-frontend/src/components/tiles/ChartTile.tsx" -ForegroundColor Cyan
...
/* ?? Base ?????????????????????????????????????????????????? */
html, body, #root { height: 100%; overflow: hidden; background: var(--bg); color: var(--text); font-family: 'IBM Plex Mono', 'Courier New', monospace; font-size: 15px; line-height: 1.4; -webkit-font-smoothing: antialiased; }
...
.tpill .lbl { font-size: 10px; color: var(--muted); text-transform: uppercase; letter-spacing: 0.08em; }
.tpill .val { font-size: 14px; font-weight: 600; }
...
.tchip .tl { font-size: 9px; color: var(--muted); text-transform: uppercase; letter-spacing: 0.06em; }
.tchip .tv { font-size: 13px; font-weight: 600; }

/* ?? Buttons ??????????????????????????????????????????????? */
.btn { padding: 5px 13px; border: 1px solid var(--bord2); border-radius: 3px; background: var(--bg2); color: var(--text); cursor: pointer; font-family: 'IBM Plex Mono', monospace; font-size: 12px; font-weight: 600; white-space: nowrap; transition: border-color 0.1s, color 0.1s, background 0.1s; }
...
.data-table thead th { position: sticky; top: 0; background: #08101a; color: var(--muted); font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 500; z-index: 2; cursor: pointer; padding: 5px 9px; }
...
.wl-cell { text-align: right; font-size: 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.wl-cell:first-child { text-align: left; font-weight: 600; font-size: 13px; }
...
.ctrl-group label { font-size: 10px; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; display: block; margin-bottom: 2px; }
...
'@
[System.IO.File]::WriteAllText((Join-Path $Root "react-frontend\src\styles\globals.css"), $c, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "[OK] react-frontend/src/styles/globals.css" -ForegroundColor Cyan

Write-Host "Building..." -ForegroundColor Yellow
$wslRoot = ($Root -replace 'C:\\\\', '/mnt/c/') -replace '\\\\', '/'
wsl.exe bash -lc "cd '$wslRoot/react-frontend' && npm run build 2>&1 | tail -15"
if ($LASTEXITCODE -eq 0) { Write-Host "[Granite] Build OK - restart app then refresh browser" -ForegroundColor Green }
else { Write-Host "[Granite] Errors above - paste to Claude" -ForegroundColor Red }
